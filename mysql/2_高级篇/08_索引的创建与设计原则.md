...
使用`参数SPATIAL`可以设置索引为`空间索引`。空间索引只能建立在空间数据类型上，这样可以提高系统获取空间数据的效率。MySQL中的空间数据类型包括`GEOMETRY`、`POINT`、`LINESTRING`和`POLYGON`等。目前只有MylSAM存储引擎支持空间检索，而且索引的字段不能为空值。对于初学者来说，这类索引很少会用到。
...
![](https://gitlab.com/eardh/picture/-/raw/main/mysql_img/202203211100285.jpeg)

从结果可以看出，索引仍然是默认的升序。

在MySQL 8.0版本中查看数据表ts1的结构，结果如下：

![](https://gitlab.com/eardh/picture/-/raw/main/mysql_img/202203211100666.jpeg)
...
![](https://gitlab.com/eardh/picture/-/raw/main/mysql_img/202203211101661.png)
